...
- Produce comparison images for the README
"""
...
def generate_video_32(dit, vae):
    ...
    for seed in [0, 7, 13, 42, 99, 123, 256, 404, 512, 1024]:
        ...
        video_np = np.clip((video[0].float().cpu().numpy() + 1.0) / 2.0, 0, 1)
    ...
    writer = imageio.get_writer(
        os.path.join(DOC_DIR, "generated_32f.mp4"),
        fps=12, codec="libx264", quality=8, pixelformat="yuv420p")
    for i in range(best_video.shape[0]):
        writer.append_data(to_uint8(best_video[i]))
    writer.close()

    # Save grid of every 4th frame (8 frames)
...
def compress_real_video(vae, video_path, max_frames=32):
    """
    Load a real video, compress at budgets {1,2,4,8,16}, compare with
    standard (p>0.5) and also a no-compression baseline (all frames).
    """
    ...
    video_model = (video_01 * 2.0 - 1.0)
    video_t = torch.tensor(video_model, device=DEVICE, dtype=torch.bfloat16).unsqueeze(0)
    ...
    latent = mean  # deterministic
    fill = vae.fill_token.to(torch.bfloat16)

    sel = selection_probs.squeeze(-1).squeeze(0).float().cpu().numpy()  # (T,)
    print(f"  Selection probs: min={sel.min():.3f}, max={sel.max():.3f}, mean={sel.mean():.3f}")

    # No-compression baseline: all frames kept
    recon_all = vae.decoder(latent, mask4d, train=False)
    recon_all_np = np.clip((recon_all[0].float().cpu().numpy() + 1.0) / 2.0, 0, 1)
    ...
    recon_std_np = np.clip((recon_std[0].float().cpu().numpy() + 1.0) / 2.0, 0, 1)
    ...
    for budget in [16, 8, 4, 2, 1]:
        ...
        recon_np = np.clip((recon[0].float().cpu().numpy() + 1.0) / 2.0, 0, 1)
...
def save_compression_comparison(video_01, rows):
    """Build the big comparison figure."""
    T = video_01.shape[0]
    ...
    all_labeled_rows.append(("Original (32 frames)", orig_grid))

    # All frames reconstruction
    recon_np, mse, _, _ = rows["all"]
    grid = make_grid([to_uint8(recon_np[i]) for i in display_idx], nrow=nframes, pad=1)
    all_labeled_rows.append((f"All 32 frames kept  (MSE={mse:.4f})", grid))

    # Budgets from most to fewest
    for budget in [16, 8, 4, 2, 1]:
        recon_np, mse, kept, _ = rows[budget]
        grid = make_grid([to_uint8(recon_np[i]) for i in display_idx], nrow=nframes, pad=1)
        all_labeled_rows.append((f"Top-{budget} frames kept  (MSE={mse:.4f})", grid))

    # Standard
    recon_np, mse, _, kept = rows["standard"]
    grid = make_grid([to_uint8(recon_np[i]) for i in display_idx], nrow=nframes, pad=1)
    all_labeled_rows.append((f"Standard p>0.5 ({kept}f kept)  (MSE={mse:.4f})", grid))
    ...
    canvas = canvas[:y]
    imageio.imwrite(os.path.join(DOC_DIR, "compression_comparison.png"), canvas)
    print(f"  Saved compression_comparison.png ({canvas.shape[1]}x{canvas.shape[0]})")
...
def evaluate_mse(vae, video_dir, num_videos=50, max_frames=32):
    ...
    all_mse = []
    all_mae = []
    # Pick a visually interesting video for the side-by-side
    best_vid = None
    best_rec = None
    best_contrast = -1
    ...
    for idx, path in enumerate(paths):
        ...
        video_t = torch.tensor(video_01 * 2.0 - 1.0, device=DEVICE, dtype=torch.bfloat16).unsqueeze(0)
        mask4d = torch.ones(1, 1, 1, T, dtype=torch.bool, device=DEVICE)
        ...
        # Track most visually interesting (highest contrast) video
        c = video_01.std()
        if c > best_contrast:
            best_contrast = c
            best_vid = video_01
...
def save_recon_comparison(video_01, recon_np):
    ...
    diff = np.abs(video_01 - recon_np[:T])
    diff_amp = np.clip(diff * 5.0, 0, 1)
    diff_grid = make_grid([to_uint8(diff_amp[i]) for i in display_idx], nrow=nframes, pad=1)
    ...
    row_w = orig_grid.shape[1]
    row_h = orig_grid.shape[0]
    ...
    canvas = canvas[:y]
    imageio.imwrite(os.path.join(DOC_DIR, "reconstruction_comparison.png"), canvas)
...
def main():
    ...
    # 1. Generate 32-frame video
    gen_video = generate_video_32(dit, vae)

    # 2. Compress a real video at various budgets
    # Pick a video with good visual content
    ...
    for f in sorted(os.listdir(DOC_DIR)):
        sz = os.path.getsize(os.path.join(DOC_DIR, f))
